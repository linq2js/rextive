...
import { GameMenu, ThemeSelector, MathCategoryIcons, type DifficultyOption, type ThemeOption } from "@/components/GameMenu";
...
const MIXABLE_CATEGORIES: Exclude<MathCategory, "mix">[] = ["quiz", "compare", "fillBlank", "sort", "chain"];
...
type MathProblem = QuizProblem | CompareProblem | FillBlankProblem | SortProblem | ChainProblem;
...
const CATEGORY_INFO: Record<MathCategory, { name: string; description: string; color: string }> = {
  mix: { name: "Mix", description: "All categories!", color: "from-fuchsia-400 to-violet-500" },
  quiz: { name: "Quiz", description: "Multiple choice", color: "from-blue-400 to-cyan-500" },
  compare: { name: "Compare", description: "< > =", color: "from-purple-400 to-pink-500" },
  fillBlank: { name: "Fill Blank", description: "Find missing", color: "from-amber-400 to-orange-500" },
  sort: { name: "Sort", description: "Order numbers", color: "from-emerald-400 to-green-500" },
  chain: { name: "Chain", description: "Multi-step", color: "from-red-400 to-rose-500" },
};
...
function generateOptions(correctAnswer: number, count: number = 4): number[] {
  ...
  while (options.size < count) {
    ...
    if (options.size < count && correctAnswer - fallback >= 0 && !options.has(correctAnswer - fallback)) {
      options.add(correctAnswer - fallback);
    }
    fallback++;
  }
...
function generateQuizProblem(difficulty: Difficulty): QuizProblem {
  ...
  switch (operation) {
    ...
    case "-":
      num1 = randomInt(config.numRange.min, config.numRange.max);
      num2 = randomInt(config.numRange.min, Math.min(num1, config.numRange.max));
      answer = num1 - num2;
      break;
    case "×":
  ...
  return { category: "quiz", num1, num2, operation, answer, options: generateOptions(answer) };
}
...
function generateCompareProblem(difficulty: Difficulty): CompareProblem {
  ...
  switch (operation) {
    ...
    case "-":
      leftNum1 = randomInt(config.numRange.min, config.numRange.max);
      leftNum2 = randomInt(config.numRange.min, Math.min(leftNum1, config.numRange.max));
      leftValue = leftNum1 - leftNum2;
      break;
    case "×":
  ...
  if (Math.random() > 0.5) {
    ...
    const rightOp = config.operations[randomInt(0, config.operations.length - 1)];
  ...
  const answer: CompareOperator = leftValue < rightValue ? "<" : leftValue > rightValue ? ">" : "=";
  
  return { category: "compare", leftExpr, leftValue, rightExpr, rightValue, answer };
}
...
function generateFillBlankProblem(difficulty: Difficulty): FillBlankProblem {
  ...
  let num1: number, num2: number, result: number, blankPosition: number, answer: number, expression: string;
  ...
  return { category: "fillBlank", expression, answer, options: generateOptions(answer) };
}
...
function generateSortProblem(difficulty: Difficulty): SortProblem {
  ...
  const sortedNumbers = [...numbersArray].sort((a, b) => ascending ? a - b : b - a);
...
function generateChainProblem(difficulty: Difficulty): ChainProblem {
  ...
  let current = randomInt(config.numRange.min, Math.min(20, config.numRange.max));
  ...
  return { category: "chain", expression, answer: current, options: generateOptions(current) };
}

function generateProblem(category: MathCategory, difficulty: Difficulty): MathProblem {
  // For "mix" mode, randomly select from all categories
  const actualCategory = category === "mix" 
    ? MIXABLE_CATEGORIES[randomInt(0, MIXABLE_CATEGORIES.length - 1)]
    : category;
  
  switch (actualCategory) {
    case "quiz": return generateQuizProblem(difficulty);
    case "compare": return generateCompareProblem(difficulty);
    case "fillBlank": return generateFillBlankProblem(difficulty);
    case "sort": return generateSortProblem(difficulty);
    case "chain": return generateChainProblem(difficulty);
  }
}
...
function mathQuestLogic() {
  ...
  const currentProblem = signal<MathProblem | null>(null, { name: "math.currentProblem" });
  ...
  const answerStatus = signal<"correct" | "wrong" | null>(null, { name: "math.answerStatus" });
  ...
  function submitAnswer(answer: number) {
    ...
    if (problem.category !== "quiz" && problem.category !== "fillBlank" && problem.category !== "chain") return;
  ...
  function submitCompare(op: CompareOperator) {
    const problem = currentProblem();
    if (!problem || answerStatus() !== null || problem.category !== "compare") return;
  ...
  function addToSortOrder(num: number) {
    const problem = currentProblem();
    if (!problem || answerStatus() !== null || problem.category !== "sort") return;
    ...
    if (newOrder.length === problem.numbers.length) {
      const isCorrect = newOrder.every((n, i) => n === problem.sortedNumbers[i]);
      if (isCorrect) {
        handleCorrectAnswer();
      } else {
        handleWrongAnswer();
  ...
  async function finishGame() {
    ...
    if (profile) {
      await gameProgressRepository.recordGameSession(profile.id, "math-quest", score(), 1);
    }
  }
  ...
  return {
    gameState, difficulty, category, currentProblem, problemIndex,
    timeLeft, score, streak, bestStreak, correctCount, wrongCount,
    answerStatus, sortUserOrder,
    energy: $energy.energy,
    profile: $profile.profile,
    setDifficulty: (d: Difficulty) => difficulty.set(d),
    setCategory: (c: MathCategory) => category.set(c),
    startGame, submitAnswer, submitCompare, addToSortOrder, clearSortOrder,
    tick, backToMenu,
  };
}
      ...
      return (
        <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-blue-500 to-cyan-600 p-4">
          <div className="card text-center bg-white/95">
            ...
            <Link to="/" viewTransition className="btn bg-blue-500 text-white mt-4 hover:bg-blue-600">
              Go Home
            </Link>
          </div>
        </div>
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-500 via-cyan-500 to-teal-500 p-4 pb-12 safe-bottom">
        <header className="mb-4">
          <div className="mx-auto max-w-3xl flex items-center justify-between">
            <Link to="/dashboard" viewTransition className="flex items-center gap-2 text-white/80 hover:text-white transition-colors">
...
function MathIcon({ className = "w-16 h-16" }: { className?: string }) {
  return (
    <svg viewBox="0 0 64 64" className={className}>
      ...
      <text x="44" y="21" fontSize="10" fill="#1e40af" fontWeight="bold" textAnchor="end">123</text>
      ...
      <text x="18" y="35" fontSize="7" fill="white" fontWeight="bold">7</text>
      <text x="32" y="35" fontSize="7" fill="white" fontWeight="bold">8</text>
      <text x="46" y="35" fontSize="7" fill="white" fontWeight="bold">+</text>
      <text x="18" y="47" fontSize="7" fill="white" fontWeight="bold">4</text>
      <text x="32" y="47" fontSize="7" fill="white" fontWeight="bold">5</text>
      <text x="46" y="47" fontSize="7" fill="white" fontWeight="bold">−</text>
      <text x="25" y="59" fontSize="7" fill="white" fontWeight="bold">0</text>
      <text x="46" y="59" fontSize="7" fill="white" fontWeight="bold">=</text>
    </svg>
  );
}
...
function TrophyIcon({ className = "w-16 h-16" }: { className?: string }) {
  return (
    <svg viewBox="0 0 48 48" className={className}>
      <path d="M38 8H34V6C34 4.9 33.1 4 32 4H16C14.9 4 14 4.9 14 6V8H10C7.79 8 6 9.79 6 12V14C6 18.42 9.58 22 14 22C14.36 22 14.71 21.97 15.06 21.92C16.59 25.15 19.5 27.52 23 28.28V34H18C16.9 34 16 34.9 16 36V42C16 43.1 16.9 44 18 44H30C31.1 44 32 43.1 32 42V36C32 34.9 31.1 34 30 34H25V28.28C28.5 27.52 31.41 25.15 32.94 21.92C33.29 21.97 33.64 22 34 22C38.42 22 42 18.42 42 14V12C42 9.79 40.21 8 38 8Z" fill="#fbbf24" />
      <path d="M14 18C11.79 18 10 16.21 10 14V12H14V18ZM34 12H38V14C38 16.21 36.21 18 34 18V12Z" fill="#f59e0b" />
    </svg>
  );
}
...
function StarRatingIcon({ filled }: { filled: boolean }) {
  return (
    <svg viewBox="0 0 24 24" className="w-10 h-10">
      <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" fill={filled ? "#fbbf24" : "#374151"} className={filled ? "drop-shadow-lg" : "opacity-30"} />
    </svg>
  );
}
...
const MATH_DIFFICULTY_OPTIONS: DifficultyOption[] = [
  { value: "easy", label: "Easy", description: "Numbers 1-10", color: "from-emerald-400 to-green-500" },
  { value: "medium", label: "Medium", description: "Numbers 1-20", color: "from-amber-400 to-orange-500" },
  { value: "hard", label: "Hard", description: "Numbers 5-50", color: "from-red-400 to-rose-500" },
];

const CATEGORY_OPTIONS: ThemeOption[] = Object.entries(CATEGORY_INFO).map(([key, info]) => ({
  value: key,
  label: info.name,
  color: info.color,
...
const HOW_TO_PLAY: Record<MathCategory, string[]> = {
  mix: ["Random challenge mode!", "Each problem is a different type", "Quiz, Compare, Fill, Sort & Chain!"],
  quiz: ["Solve math problems!", "Tap the correct answer", "Build streaks for bonus points"],
  compare: ["Compare two sides!", "Choose < (less), > (greater), or = (equal)", "Think fast!"],
  fillBlank: ["Find the missing number!", "Complete the equation", "All operations included"],
  sort: ["Arrange numbers in order!", "Tap numbers from smallest to largest (or reverse)", "Clear to retry"],
  chain: ["Solve step-by-step!", "Calculate the chain of operations", "Follow order: left to right"],
};
...
function PlayingScreen({ $game }: { $game: ReturnType<typeof mathQuestLogic> }) {
  return rx(() => {
      ...
      <div className="space-y-4">
        ...
        <div className="card bg-white/95 backdrop-blur">
          <div className="flex items-center justify-between text-sm">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <span className="ml-1 font-bold">{problemIndex + 1}/{config.problemCount}</span>
              </div>
              {streak > 0 && (
          ...
          <div className="mt-2 text-xs text-center">
            <span className={`px-2 py-0.5 rounded-full bg-gradient-to-r ${catInfo.color} text-white`}>
              {catInfo.name}
            </span>
          </div>
          <div className="mt-2 h-2 rounded-full bg-gray-200 overflow-hidden">
            <div className="h-full bg-gradient-to-r from-blue-400 to-cyan-500 transition-all duration-300" style={{ width: `${(correctCount / config.problemCount) * 100}%` }} />
          </div>
        </div>

        {/* Problem UI - varies by category */}
        {problem.category === "quiz" && <QuizProblemUI problem={problem} $game={$game} answerStatus={answerStatus} />}
        {problem.category === "compare" && <CompareProblemUI problem={problem} $game={$game} answerStatus={answerStatus} />}
        {problem.category === "fillBlank" && <FillBlankProblemUI problem={problem} $game={$game} answerStatus={answerStatus} />}
        {problem.category === "sort" && <SortProblemUI problem={problem} $game={$game} answerStatus={answerStatus} />}
        {problem.category === "chain" && <ChainProblemUI problem={problem} $game={$game} answerStatus={answerStatus} />}
      </div>
    );
  });
}
...
function QuizProblemUI({ problem, $game, answerStatus }: { problem: QuizProblem; $game: ReturnType<typeof mathQuestLogic>; answerStatus: "correct" | "wrong" | null }) {
  return (
    <>
      <ProblemCard answerStatus={answerStatus} correctAnswer={problem.answer}>
        <div className="text-5xl sm:text-6xl font-bold text-gray-800 mb-2 font-display">
          {problem.num1} {problem.operation} {problem.num2}
        </div>
        <div className="text-2xl text-gray-500">= ?</div>
      </ProblemCard>
      <OptionsGrid options={problem.options} correctAnswer={problem.answer} answerStatus={answerStatus} onSelect={(n) => $game.submitAnswer(n)} />
    </>
  );
}

function CompareProblemUI({ problem, $game, answerStatus }: { problem: CompareProblem; $game: ReturnType<typeof mathQuestLogic>; answerStatus: "correct" | "wrong" | null }) {
  ...
  return (
    <>
      <ProblemCard answerStatus={answerStatus} correctAnswer={problem.answer}>
        ...
        <div className="text-4xl sm:text-5xl font-bold text-blue-500 my-2">?</div>
          ...
          return (
            <button
              ...
              className={`py-5 px-4 rounded-2xl text-4xl font-bold transition-all ${
                isCorrect ? "bg-green-500 text-white scale-105 shadow-lg" :
                isWrong ? "bg-gray-200 text-gray-400" :
                answerStatus !== null ? "bg-gray-200 text-gray-400" :
...
function FillBlankProblemUI({ problem, $game, answerStatus }: { problem: FillBlankProblem; $game: ReturnType<typeof mathQuestLogic>; answerStatus: "correct" | "wrong" | null }) {
  return (
    <>
      <ProblemCard answerStatus={answerStatus} correctAnswer={problem.answer}>
        <div className="text-4xl sm:text-5xl font-bold text-gray-800 font-display">
          {problem.expression.split("_").map((part, i, arr) => (
            <span key={i}>
              {part}
              {i < arr.length - 1 && <span className="text-blue-500 mx-1">?</span>}
            </span>
          ))}
        </div>
      </ProblemCard>
...
function SortProblemUI({ problem, $game, answerStatus }: { problem: SortProblem; $game: ReturnType<typeof mathQuestLogic>; answerStatus: "correct" | "wrong" | null }) {
  const userOrder = $game.sortUserOrder();
  const availableNumbers = problem.numbers.filter(n => !userOrder.includes(n));
  
  return (
    <>
      <ProblemCard answerStatus={answerStatus} correctAnswer={problem.sortedNumbers.join(", ")}>
        ...
        <div className="flex flex-wrap justify-center gap-2 min-h-14 p-3 bg-gray-100 rounded-xl">
          {userOrder.length === 0 ? (
            ...
            userOrder.map((n, i) => (
              <span key={i} className="px-4 py-2 bg-blue-500 text-white rounded-lg text-xl font-bold">
                {n}
              </span>
            ))
          )}
      ...
      {userOrder.length > 0 && answerStatus === null && (
        <button onClick={() => $game.clearSortOrder()} className="w-full py-2 text-gray-500 hover:text-gray-700">
          ↺ Clear & Retry
        </button>
      )}
    </>
...
function ChainProblemUI({ problem, $game, answerStatus }: { problem: ChainProblem; $game: ReturnType<typeof mathQuestLogic>; answerStatus: "correct" | "wrong" | null }) {
  return (
    <>
      <ProblemCard answerStatus={answerStatus} correctAnswer={problem.answer}>
        <div className="text-3xl sm:text-4xl font-bold text-gray-800 mb-2 font-display">
...
function ProblemCard({ children, answerStatus, correctAnswer }: { children: React.ReactNode; answerStatus: "correct" | "wrong" | null; correctAnswer: string | number }) {
  return (
    <div className={`card bg-white/95 text-center py-6 transition-all duration-300 ${
      answerStatus === "correct" ? "ring-4 ring-green-400 bg-green-50" :
      answerStatus === "wrong" ? "ring-4 ring-red-400 bg-red-50" : ""
    }`}>
      {children}
      ...
      <div className="h-16 flex items-center justify-center mt-2">
        {answerStatus === "correct" ? (
          <CheckIcon className="w-12 h-12 text-green-500 animate-bounce" />
        ) : answerStatus === "wrong" ? (
          <div className="flex flex-col items-center text-red-500">
            <XIcon className="w-10 h-10" />
            <span className="text-sm font-medium">Answer: {correctAnswer}</span>
          </div>
        ) : (
          <div className="w-12 h-12 opacity-0"><CheckIcon className="w-12 h-12" /></div>
        )}
      </div>
    </div>
  );
...
function OptionsGrid({ options, correctAnswer, answerStatus, onSelect }: { options: number[]; correctAnswer: number; answerStatus: "correct" | "wrong" | null; onSelect: (n: number) => void }) {
  return (
...
function FinishedScreen({ $game }: { $game: ReturnType<typeof mathQuestLogic> }) {
  return rx(() => {
    const score = $game.score();
    const correctCount = $game.correctCount();
    ...
    const stars = calculateStarRating(correctCount, config.problemCount, accuracy, bestStreak);
    const baseXP = score;
    const timeBonus = completed && timeLeft > 0 ? Math.round(timeLeft * 0.5) : 0;
    const totalXP = baseXP + timeBonus;

    return (
      <div className="space-y-6">
        <div className="card text-center py-8 bg-white/95">
          {completed && accuracy >= 70 ? <TrophyIcon className="w-20 h-20 mx-auto" /> : <MathIcon className="w-20 h-20 mx-auto" />}
          <h2 className="font-display text-2xl font-bold text-gray-800 mt-4">
            {completed ? (accuracy >= 80 ? "Great Job!" : "Well Done!") : "Time's Up!"}
          </h2>
          
          <div className="mt-2 text-sm text-gray-500">{CATEGORY_INFO[category].name} Mode</div>

          <div className="mt-4 flex justify-center gap-2">
            {[1, 2, 3].map((i) => (
              <div key={i} className={i <= stars ? "animate-bounce" : ""} style={{ animationDelay: `${i * 0.1}s` }}>
                <StarRatingIcon filled={i <= stars} />
              </div>
            ))}
          </div>

          <div className="mt-6 text-4xl font-bold text-blue-600">{score} pts</div>

          <div className="mt-6 grid grid-cols-2 gap-4 text-left">
            <StatItem label="Correct" value={`${correctCount}/${config.problemCount}`} />
          ...
          <div className="mt-6 p-4 bg-amber-50 rounded-xl">
            ...
            {timeBonus > 0 && <div className="text-xs text-amber-600">(includes +{timeBonus} time bonus)</div>}
          </div>
        </div>
        ...
        <div className="flex flex-col gap-3">
          ...
          >
            {$game.energy() > 0 ? (
              <span className="flex items-center justify-center gap-2">Play Again <EnergyIcon /> 1</span>
            ) : "No Energy Left"}
          </button>
...
function StatItem({ label, value, icon }: { label: string; value: string | number; icon?: React.ReactNode }) {
  return (
    <div className="p-3 bg-gray-50 rounded-lg">
      ...
      <div className="font-bold text-gray-800 flex items-center gap-1">{icon}{value}</div>
    </div>
  );
}

...
export function DevToolsPanel(): React.ReactElement | null {
  ...
  // Helper to navigate to a signal - filters by name and expands it
  const navigateToSignal = useCallback((signalId: string, signalName: string) => {
    // If signal name starts with #, enable showAutoGenerated so it's visible
    if (signalName.startsWith("#")) {
      setShowAutoGenerated(true);
    }
    // Use name: prefix to filter precisely to this signal
    setSearchQuery(`name:${signalName}`);
              ...
              {isExpanded && (
                ...
                >
                  ...
                  {(() => {
                    ...
                    const deps = !info.disposed
                      ? getDependencies(dependencyGraph, info.id)
                      : [];
                    const dependents = !info.disposed
                      ? getDependents(dependencyGraph, info.id)
                      : [];
                    ...
                    return (
                      <div style={{ marginBottom: "6px" }}>
                        {/* Dependencies (what this signal depends on) */}
                        {hasDeps && (
                          <div style={{ marginBottom: hasDependents ? "8px" : 0 }}>
                            <div
                              style={{
                                fontSize: "9px",
                                color: styles.colors.textMuted,
                            ...
                            >
                              {deps.map((dep) => {
                                const depInfo = signals.get(dep.id);
                                const depValue = depInfo?.signal?.tryGet?.();
                                return (
                                  <div
                                    key={dep.id}
                                    style={{
                                    ...
                                    onClick={(e) => {
                                      e.stopPropagation();
                                      navigateToSignal(dep.id, dep.name);
                                    }}
                                    title={`${dep.name}\nUID: ${dep.id}\nClick to navigate`}
                                  >
                                    <span
                                      style={{
                                        ...styles.badgeStyles(dep.kind),
                                        fontSize: "8px",
                                        padding: "1px 3px",
                                        ...(dep.disposed && {
                                          backgroundColor: "#444",
                                    ...
                                    {!dep.disposed && depValue !== undefined && (
                                      <span
                                        style={{
                                          color: styles.colors.textMuted,
                                          maxWidth: "80px",
                        ...
                        {/* Dependents (what depends on this signal) */}
                        {hasDependents && (
                          <div>
                            <div
                              style={{
                            ...
                            >
                              {dependents.map((dependent) => {
                                const depInfo = signals.get(dependent.id);
                                const depValue = depInfo?.signal?.tryGet?.();
                                return (
                                  <div
                                    key={dependent.id}
                                    ...
                                    onClick={(e) => {
                                      e.stopPropagation();
                                      navigateToSignal(dependent.id, dependent.name);
                                    }}
                                    title={`${dependent.name}\nUID: ${dependent.id}\nClick to navigate`}
                                  >
                                    <span
                                      style={{
                                        ...styles.badgeStyles(dependent.kind),
                                        fontSize: "8px",
                                        padding: "1px 3px",
                                        ...(dependent.disposed && {
                                          backgroundColor: "#444",
                                    ...
                                    {!dependent.disposed && depValue !== undefined && (
                                      <span
                                        style={{
                                          color: styles.colors.textMuted,
                                          maxWidth: "80px",

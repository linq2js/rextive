...
import { TabBar, TabContent } from "./components/shared";
import {
  parseSearchQuery,
  matchesSignalSearch,
  matchesTagSearch,
...
type Tab = "signals" | "tags" | "events" | "chains" | "graph";
...
export function DevToolsPanel(): React.ReactElement | null {
  ...
  const updateActiveTab = useCallback((tab: Tab) => {
    if (tab === "graph") {
      // Open graph modal instead of switching to graph tab
  ...
  const renderContent = () => {
    ...
    switch (activeTab) {
      ...
      case "chains":
        return renderChains();
      case "graph":
        // Graph tab doesn't show content - clicking tab opens modal
        return null;
    }
  };
  ...
  // Helper to render signal right filters (bookmarks, sort, auto toggle)
  const renderSignalRightFilters = () => {
    return (
        ...
        >
          ...
          {isExpanded && (
            <>
              <TabBar
                tabs={(
                  ["signals", "tags", "events", "chains", "graph"] as Tab[]
                ).map((tab) => ({
                  id: tab,
              ...
              <TabContent
                ...
                filterBar={
                  activeTab === "signals"
                    ? {
                        leftFilters: renderSignalKindFilters(),
                        rightFilters: renderSignalRightFilters(),
                      }
                    : activeTab === "events"
                    ? {
                        leftFilters: renderEventKindFilters(),
                        rightFilters: (
                          <button
                            style={{
                              padding: "3px 8px",
        ...
        <GraphModal
          ...
          onNodeClick={(nodeId) => {
            setGraphSelectedNodeId(
              nodeId === graphSelectedNodeId ? null : nodeId
            );

...
export function devLog(message: string, ...args: any[]): void {
  if (isDev()) {
    console.log(`[rxblox] ${message}`, ...args);
  }
}
 ...
 * // Production (with __DEV__ defined): removed entirely
 * // Development: console.warn("[rxblox] Deprecated API used")
 * ```
 */
export function devWarn(message: string, ...args: any[]): void {
  if (isDev()) {
    console.warn(`[rxblox] ${message}`, ...args);
  }
}
 ...
 * // Production (with __DEV__ defined): removed entirely
 * // Development: console.error("[rxblox] Invalid configuration:", config)
 * ```
 */
export function devError(message: string, ...args: any[]): void {
  if (isDev()) {
    console.error(`[rxblox] ${message}`, ...args);
  }
}
...
export function devAssert(condition: boolean, message: string): void {
  if (isDev()) {
    if (!condition) {
      throw new Error(`[rxblox] Assertion failed: ${message}`);
    }
  }
}
